...
const timeUtils_1 = require("../src/shared/utils/timeUtils");
const testCases = [
    {
        name: 'same-day end',
...
testCases.forEach((test) => {
    const actual = (0, timeUtils_1.ensureShiftEndAfterStart)(test.start, test.end);
    assert_1.default.strictEqual(actual, test.expectedEnd, `${test.name}: expected ${test.expectedEnd} but got ${actual}`);
});
